...
\begin{document}
    ...
    \begin{enumerate}
        ...
        Порождающая её последовательность $f(x_n)$ $\lim_{n\to\infty} f(x_0) = f(x_0)$
        \item Определение Коши\\
        Функция $f(x)$ называется непрерывной в точке $x_0$ $\forall \varepsilon > 0$ $\exists$ $\delta = \delta(\varepsilon) > 0 : |x - x_0| < \delta \implies |f(x) - f(x_0)| < \varepsilon$
    \end{enumerate}
    Функция $f(x)$ называется непрерывной в точке $x_0$ справа (слева), если 
    \[ \lim_{x \to x_0+0} f(x) = f(x_0) \]
    \[ \left(\lim_{x \to x_0-0} f(x) = f(x_0)\right) \]
    \underline{Утверждение:} если функции $f(x)$ и $g(x)$ называются непрерывными в точке $x_0$, то их сумма, разность и произведение тоже непрерывны в точке $x_0$. При условии, что $g(x) \ne 0$, частность $\frac{f(x)}{g(x)}$ тоже непрерывна в точке $x_0$.\par\noindent
    Если функция $f(x)$ не является непрерывной в точке $x_0$, то говорят, что функция в точке $x_0$ \textbf{не имеет разрыв}.\\
    ...
    \end{enumerate}
    %Отформачь все пж
    \subsection*{Определние}
    Функция $y=f(x)$ заданная в $u(x_0)$ называется дифферинцируемой в этой (.) если
    ее приращение $\Delta y = f(x_0+\Delta x)=f(x_0)$ рпедставление в этой окружностьи в виде
    $\Delta y = A \Delta x = o(\Delta x), \Delta x => 0, A = const$\\
    \subsection*{Определние}
    Линейная часть прирощения ф-ии $A\Delta x $ называется дифферинциалом ф-ии в (.)
    $x_0$ и обозначается df|$x=x_0$ или df($x_0$)\\
    Тогда $\Delta y = dy+o(\Delta x)$\\
    \subsection*{Замечание}
    $o(\Delta x)=o(A\Delta x)=o(dy)$\\
    ...
    \subsection*{Теорема 9.1(необходимость и достаточность условия диф-ии в (.))}
    Для того чтобы f(x) была диф-ема в (.) $x_0$ чтобы в этой (.) она имела конечную производную
    \subsection*{Док-во}
    => $\sqsupset$ f(x) диф-ема в (.) $x_0$\\
    Покажем что $\exists$ $f'(x_0)$ - конечное\\
    Тогда по \underline{Опр} $\Delta y = A \Delta x+o(\Delta x)$\\
    $\Delta y = A \Delta x+ \epsilon(\Delta x)*\Delta x$\\
    $\lim_{\Delta x \to 0}{\epsilon(\Delta x)}= 0$\\
    $\frac{\Delta y}{\Delta x}=A+\epsilon(\Delta x)$\\
    $lim_{\Delta x \to 0}{\frac{\Delta y}{\Delta x}}=A$\\
    $f'(x)|x=x_0=A=const$\\
    <= $\sqsupset f(x)$ имеет конечную производную в (.)$x_0$\\
    Т е $\exists \lim_{\Delta x \to 0}{\frac{\Delta y}{\Delta x}}=f'(x_0)$\\
    $\frac{\Delta y}{\Delta x}=f'(x_0)+\epsilon(\Delta x)|*\Delta x$\\
    $\Delta y=f'(x_0)\Delta x+\epsilon(\Delta x)\Delta x= f'(x_0)\Delta x+o(\Delta x)$\\
    ЧТД\\ 
    \subsection*{Замечание}
    Такие образом dy|$x=x_0=f'(x_0)\Delta x$
    \section*{Геометрический смысл дифферинцирования}
    \includegraphics[width=1\linewidth]{4.2.2.png}\\
    Рассмотрим ф-ию y=f(x)\\
    $M_0(x_0,f(x_0))$\\
    $M(x_0, \Delta x,f(x_0+\Delta x))$\\
    $\tg \alpha = f'(x_0)$\\
    Рассмотрим $\Delta ABM_0 \; (AM_0)\tg \alpha = |AB|$\\
    $f'(x_0)\Delta x = \Delta y$\\
    $|MB|=o(\Delta x)$\\
    \includegraphics[width=1\linewidth]{4.2.3.jpg}\\
    Отсюда для независимой переменной x $\Delta x = dx$\\
    Геометрический смысл дифферинцирования: это есть прирощение окружности касательной
    производной к графику окружности в (.) $x_0$\\
    $V=S(t)|t=t_0=\frac{dS}{dt}$\\
    $dS=S'(t)|t=t_0$\\
    $dt=S'(t)|t=t_0 \; \Delta t$\\
    Дифферинциал dS равен пути который прошла бы рассматриваемая точка за время $\Delta t$
    начиная с момента времени $t=t_0$, если бы на этом участке пути скорость была бы постоянно
    равной $S'=t_0$
    \subsection*{Свойства дифферинциалов}
    \begin{enumerate}
        \item $d(u+-V)=du+-dv$
        \item $d(u*v)=udv+vdu$
        \item $d(CU)=cdu$
    ...
    \end{enumerate} 
    \subsection*{Док-во}
    $d(\frac{u}{v})=(\frac{u}{v})'dx=\frac{u'v-uv'}{v^2}dx=\frac{vdu-udv}{v^2}$
    \section*{Параграф 10(Примемение дифферинциалов в приближенных вычислениях. Дифференциал сложных функций)}
    Если f(x) диф-ла в (.) $x_0$\\
    $\Delta y=dy+o(dx)$\\
    $\Delta y == dy$\\
    $f(x_0+\Delta x)-f(x_0)==f'(x_0)dx|\Delta x$
    ...
    %тест ебанной ветки
    \section*{Параграф 11(Производные)}
    \subsection*{Определение}
    $\sqsupset$ функция y=f(x) имеет производную $y'=f(x)$ во всех (.) некоторой окрестности
    (.) $x_0$. Если функция f(x) имеет в (.) $x_0$ производную [(f(x))']|$x=x_0$, то оно называется
    второй производной функцией f(x) в (.) $x_0$\\
    f''=(f')'\\
    Аналогично \\
    $f^n=(f^(m+1))'$%большая часть пропущена, спиздить у кого-то записи
    \begin{enumerate}
        \item $y=a^x$\\
        $y''=a^x\ln^2(a)$\\
        $y^n=a^x\ln^n(a)$
        \item $y=\sin(x)$\\
        $y(n)=sin(x)=sin(\frac{n\pi}{2}+x)$
        \item $y=\cos(x)$\\
        $cos^n=cos(\frac{n\pi}{2}+x)$
        \item $y=ln(x)$
        
    \end{enumerate}
    %выше не дописано, спиздить записи за прошлую лекцию(15.12)
    %максон дал лекцию щищ

    \subsection*{\underline{ЗМ}}
    (.) в которой функция достигает экстремального значения является
    внутренней (.) интервала
    \subsection*{Т 12.2(Теорема Ройля)}
    Если f(x) непрерывна на [a,b], дифферинцируема на интервале [a,b] и
    f(a) = f(b). Тогда на (a;b) $\exists$ по крайней мере одна (.) $\xi \in$ (a,b) 
    f($\xi$)=0
    \subsection*{Док-во}
    \begin{enumerate}
        \item f=const, Тогда $\forall \in (a,b) f'(\xi)=0$ ЧТД
        \item $f \not = const$, Тогда по 2-ой Теореме Вейерштрасса $\exists (.) x_1,x_2$ в которых
        функция достигает свои набольшие и наименьшие значения
    \end{enumerate}
    \subsection*{Теорема 12.3(Теорема Коши)}
    Если функция f(x) и g(x) непрерывна на отрезке [a,b], дифферинцируема на (a;b),g'(x)$\not =0 \forall
    x \in (a;b)$ Тогда $\exists (.) \xi \in (a,b)$\\
    $\frac{f(b)-f(a)}{g(b)-g(a)}=\frac{f'(\xi)}{g'(\xi)}$
    \subsection*{Док-во}
    \underline{ЗМ} $g(b)\not=g(a)$ \\
    Тк если бы это было так, то g(x) удовлетворяло бы условию Теоремы Ройля => $\exists (.) \xi
    \in (a;b) g(\xi)=0$, а это не так\\
    Составим вспомогательную функцию\\
    $F(x)=f(x)-f(a)-\frac{f(b)-f(a)}{g(b)-g(a)}(g(x)-g(a))$\\
    Тк f(x),g(x) непрерывна на [a,b] и диф-ма на (a;b)  => F(x) непрерывна
    на [a,b] и диф-ма на (a;b)\\
    $F(a)=0\\F(b)=0$\\Те f(x) удовлетворяет условию Теоремы Райля => $\exists (.) \xi
    \in (a;b) F(\xi) =0$\\
    Тогда F'(x)=$f(x)-\frac{f(b)-f(a)}{g(b)-g(a)}g(x)\\F'(\xi)=0 =>f'(\xi)-\frac{f(b)-f(a)}{g(b)-g(a)}g(\xi)=0$\\
    $\frac{f(b)-f(a)}{g(b)-g(a)}g'(\xi)=f'(\xi)| g'(\xi)\not=0$\\
    $\frac{f(b)-f(a)}{g(b)-g(a)}=\frac{f'(\xi)}{g'(\xi)}$ ЧТД
    \subsection*{Теорема 12.4}
    Если f(x) непрерывна на [a,b] диф-ма на (a;b) тогда$\exists (.) \xi \in (a;b) \; f(b)-f(a)=
    f'(\xi)^{b-a}$
    \subsection*{Док-во}
    Если в Теореме Коши (12.3) принять g(x)=x => ЧТД %че за хуйня
    \subsection*{Геометрический смысл теоремы Лагранжа}%фотка в мобиле добавить
    $\frac{f(b)-f(a)}{b-a}= tg \alpha A=f'(\xi)$\\
    $\exists (.) \xi \in (a;b) касательная к графику функции в этой (.) || хорде AB$\\
    \underline{ЗМ} Промежуточное значени $\xi$ можно записать в виде
    $\xi=a+b(b-a) ;\; 0<b<1$\\
    Тогда \underline{$f(b)-f(a)=f'(a+o)(b-a)(b-a)$}\\
    $\sqsupset b-a=\Delta x$\\
    $f(b)-f(a)=f(x+\Delta x)-f(x)$\\
    Тогда\\ $f(x+\Delta x) -f(x)=f'(x+B\Delta x)\Delta x$\\
    Все три функции называются функцией формулых конечных приращения Лоранжа
    \subsection*{Т 12.5(Постоянных отрезка че-то там(В пт скажет))}
    %лекция 26.12
    Рассмотрим производную многочлена в степени n\\
    $P_n(x)=b_0+b_1x+b_2x'+...+b_nx^n=\sum\limits_{k=0}^n b_k*x^k$\\
    $\sqsupset x_0 - произвольное число$\\
    $x=(x-x_0)+f_0$\\
    Тогда $\sum\limits_{k=0}^n b_n((x-x_0)+x_0)^k$ После возведения в степень и приведения подобных слагаемых
    , получим $P_n(x)=a_0+a_1(x-x_0)+a_2(x-x_0)^2+...+a_n(x-x_0)^n=\sum\limits_{k=0}^n a_k(x-x_0)^k$\\
    $P'_n(x)=a_1+2*a_2+3*a_3(x-x_0)^2+...+na_n(x-x_0)^{n-1}$\\
    $P''_n(x)=2*a_2+3*2*a_2(x-x_0)+4*3a_4(x-x_0)^2+...+n(n-1)(x-x_0)^n-2$\\
    $P_n^{(k)}=1*2*3...k*a_k+...+(n-k+1)...(n-1)n*a_n(x-x_0)^{n-k}$\\
    $P_n^{(n)}(x)=n!a_n$\\
    $a_k=\frac{P_k^{(k)}(x_0)}{k!}$\\
    Многочлен $P_n(x)$ можно разложить по степеням (x-$x_0$) единственным образом\\
    $P_n(x)=P_n(x_0)+\frac{P_n(x_0)}{1!}(x-x_0)+\frac{P_n(x_0)}{2!}(x-x_0)+...+\frac{P_n^{(n)}(n)}{n!}(x-x_0)=
    \sum\limits_{k=0}^n \frac{P^{(n)}(x_0)}{k!}$\\
    \subsection*{Пример}
    Записать данный многочлен по степеням x+1
    \begin{math}
        P(x)=1+3x+5x^2-2x^3\\
        x_0=-1\\
        P'(x)=3+10x-6x^2\\
    ...
    \end{math}
    \section*{Параграф 15(Формула Тейлора для функций)}
    Рассмотрим функцию f(x) имееющею непрерывную производную до $(n+1)$ порядка в $u(x_0)$\\
    Составим многочлен $Q_n(x)=f(x_0)+\frac{f'(x_0)}{1!}(x-x_0)+\frac{f''(x_0)}{2!}(x-x_0)+...+\frac{f^{(n)}(x_0)}{n!}(x-x_0)=
    \sum\limits_{k=0}^n \frac{f^{(k)(x)}}{k!}(x-x_0)$\\
    Этот многочлен совпадает с функцией f(x) только в точке $x_0$, но !!! в остальных точках он не равен f(x)\\
    $Q_n'(x)=f'(x_0)+\frac{2}{2!}(x-x_0)+...$\\
    $Q_n'(x_0)=f(x_0)$\\
    Аналогично $Q_n''(x_0)=f''(x_0)$\\
    \\
    $\sqsupset f(x)=Q_n(x)+r_n(x)$ - формула Тейлора для функций F(x)\\
    $r_n(x)$ - погрешность при замене f(x) на $Q_n(x)$\\
    %сказал найдем чето там не услышал кекв
    $f(x_0)=Q_n(x_0) => r_0(x_0)=0$\\
    $f'(x_0)=Q_n'(x_0) => r_n'(x_0)=0$\\
    $f^(n)(x_0)=Q_n^(n)(x_0) => r_n^(n)(x_0)=0$\\
    Введем вспомогательную функцию $\phi(x)$\\
    $\phi(x)=(x-x_0)^{n+1}$\\
    $\phi(x_0)=0$ \\
    $\phi'(x_0)=0$\\
    $\phi''(x_0)=0$\\
    ...
    $\phi^{n+1}(x_0)=(x+1)'$\\
    \\
    По теореме Коши (Т 12.3)\\
    $\frac{r_n(x)}{\phi (x)}=\frac{r_n(x)-r_n(x_0)}{\phi(x)-\phi(x_0)}=\frac{r_n'(x_1)}{\phi(x_1)}
    =\frac{r'_n(x_1)-r'_n(x_0)}{\phi(x_1)-\phi(x_0)}=\frac{r'_n(x_2)}{\phi'(x_2)}=
    ...=\frac{r_n^(n)(x_n)}{\phi^(n)(x_n)}=\frac{r_n^(n)(x_n)-r_n^(n)(x_0)}{\phi^(n)(x_n)-\phi^(n)(x_0)}$\\
    $x \in (x_0,x)$ или $x_1 \in (x,x_0)$
    Тогда $r_n(x)=\frac{\phi(x)-r_n^{(n+1)}(\xi)}{\phi^{(n+1)}(\xi)}$
    %за 20 минут до конца пары я заебался чо-та, так что придется у кого-то пиздить тетрадку
    \end{document}